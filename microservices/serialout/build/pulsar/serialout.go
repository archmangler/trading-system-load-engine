...
var credentialsDBindex, _ = strconv.Atoi(os.Getenv("CREDENTIALS_DB_ADDRESS")) //14
...
var (
	serialConsumedRequests = promauto.NewCounter(prometheus.CounterOpts{
		Name: "serial_consumer_consumed_requests_total",
		Help: "The total number of serial requests taken from load queue",
	})

	serialRequestsSuccessful = promauto.NewCounter(prometheus.CounterOpts{
		Name: "serial_consumer_successul_requests_total",
		Help: "The total number of serial processed requests",
	})

	serialRequestsFailed = promauto.NewCounter(prometheus.CounterOpts{
		Name: "serial_consumer_failed_requests_total",
		Help: "The total number of serial failed requests",
	})

	serialRequestsCancelSuccessful = promauto.NewCounter(prometheus.CounterOpts{
		Name: "serial_cancel_successul_requests_total",
		Help: "The total number of serial processed order cancel requests",
	})

	serialRequestsCancelFailed = promauto.NewCounter(prometheus.CounterOpts{
		Name: "serial_cancel_failed_requests_total",
		Help: "The total number of failed serial order cancel requests",
	})
)

func recordConsumedMetrics() {
	go func() {
		serialConsumedRequests.Inc()
...
func recordSuccessCancelMetrics() {
	go func() {
		serialRequestsCancelSuccessful.Inc()
		time.Sleep(2 * time.Second)
	}()
}
...
func recordFailedCancelMetrics() {
	go func() {
		serialRequestsCancelFailed.Inc()
		time.Sleep(2 * time.Second)
