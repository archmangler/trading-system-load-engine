...
func purgeProcessedMetadataRedis(conn redis.Conn) {
	...
	response, err := conn.Do("SELECT", dataDbIndex)

	if err != nil {

		if debug == 1 {
			fmt.Println("can't connect to redis metadata db: ", dataDbIndex)
		}

		panic(err)
	} else {
		if debug == 1 {
			fmt.Println("redis select db response: ", response, " db index = ", dataDbIndex)
		}
	}
...
func process_input_data_redis_concurrent(workerId int, jobId int) {
	...
	if taskCount == numWorkers-1 || taskCount == numWorkers {
		...
		if debug == 1 {
			fmt.Println("(process_input_data_redis_concurrent) done!")
		}
...
func getWorkAllocation(inputId string, conn redis.Conn) (ds []string, err error) {
	//modified for serial worker: i.e no lookup into metadata table
	...
	msgPayload, err := redis.Strings(conn.Do("KEYS", "*"))
...
func main() {
	...
	if err != nil {
		...
		if debug == 1 {
			fmt.Println("can't connect to redis namespace: ", dataDbIndex)
		}

		panic(err)
	} else {
		if debug == 1 {
			fmt.Println("redis select work allocation db response: ", response, dataDbIndex)
		}
	}
